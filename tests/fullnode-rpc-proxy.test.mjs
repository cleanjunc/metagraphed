// Unit tests for the isolated, account-gated fullnode RPC proxy (ADR 0021,
// #6835, workers/request-handlers/fullnode-rpc-proxy.mjs). Real
...
async function makeValidatedKeyEnv(overrides = {}) {
...
test("429s when the guess-rate limiter denies (before key validation)", async () => {
  const { env } = await makeValidatedKeyEnv({
...
test("401s on a revoked key with a distinct message", async () => {
  const { env, key } = await makeValidatedKeyEnv({ revoked_at: 12345 });
  const res = await call(
    `/rpc/v1/fullnode?authorization=${key}`,
    { body: { jsonrpc: "2.0", id: 1, method: "system_health" } },
    env,
